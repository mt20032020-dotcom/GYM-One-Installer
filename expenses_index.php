...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_expense'])) {
    ...
    if ($description !== '' && $amount > 0) {
        ...
        $stmt->bind_param("ssdsi", $description, $category, $amount, $expense_date, $userid);
        $stmt->execute();
        $stmt->close();
        $alert = 'added';
    } else {
...
$expenses = [];
$total_expenses = 0;
$stmt = $conn->prepare("SELECT id, description, category, amount, expense_date FROM expenses WHERE expense_date BETWEEN ? AND ? ORDER BY expense_date DESC, id DESC");
                                    ...
                                    <table class="table table-bordered table-striped text-center">
                                        ...
                                        <tbody>
                                            <?php if (count($expenses) > 0): ?>
                                                ...
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr><td colspan="5">No hay gastos registrados en este rango de fechas.</td></tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                                </div>

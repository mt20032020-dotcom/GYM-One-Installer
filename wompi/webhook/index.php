...
$stmt2 = $db->prepare("SELECT userid, email, celular, firstname, lastname FROM users WHERE userid = ?");
...
// ===== Confirmacion de pago por WhatsApp via Chatwoot =====
try {
    if (!empty($user['celular'])) {
        require_once '/app/includes/chatwoot_plantilla.php';
        $celW = preg_replace('/\D/','', $user['celular']);
        if (preg_match('/^3\d{9}$/', $celW)) {
            $nomW   = trim($user['lastname']);
            $planW  = $ticket['name'];
            $montoW = '$' . number_format($monto_pesos, 0, ',', '.') . ' COP';
            $vencW  = ($plan_result && $plan_result['type'] === 'active' && !empty($plan_result['end_date']))
                      ? date('d/m/Y', strtotime($plan_result['end_date']))
                      : 'se activa al terminar tu plan actual';
            $resW = @enviar_plantilla_chatwoot(
                $celW,
                trim($user['lastname'].' '.$user['firstname']),
                'confirmacion_pago',
                [$nomW, $planW, $montoW, $vencW],
                null,
                "Hola $nomW, tu pago de $montoW por $planW fue confirmado."
            );
            @file_put_contents('/app/wompi/webhook_log.txt',
                date('Y-m-d H:i:s')." WA-PLANTILLA cel=$celW ".json_encode($resW)."\n", FILE_APPEND);
        }
    }
} catch (\Throwable $eWA) {
